...
export default function MasterDataModule({ currentFY, onFYChange, activeSubModule = "account-master" }: MasterDataModuleProps) {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || `Failed to create ${getPageInfo().singular}`,
        variant: "destructive",
      });
    }
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || `Failed to update ${getPageInfo().singular}`,
        variant: "destructive",
      });
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || `Failed to delete ${getPageInfo().singular}`,
        variant: "destructive",
      });
    }
  });
...
function FormDialog({
  ...
  // Enable Enter key navigation between form fields
  useEnterKeyNavigation(showForm);
...
function AccountFormFields({ form, places }: any) {
  return (
    <>
      ...
      {/* Government Identity Details - Collapsible */}
      <Collapsible>
      ...
      {/* Bank Account Details - Collapsible */}
      <Collapsible>
        <CollapsibleTrigger className="flex items-center justify-between w-full p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">
